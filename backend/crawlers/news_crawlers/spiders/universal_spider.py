...
from ..items import NewsArticleItem, CandidateSourceItem, SourceUpdateItem
import trafilatura
...
class UniversalNewsSpider(RedisSpider):
    ...
    def make_request_from_data(self, data):
        ...
        return scrapy.Request(url, callback=self.parse_homepage, meta={"playwright": True}, dont_filter=True)
    ...
    def parse_homepage(self, response):
        ...
        try:
            ...
            self.logger.error(f"Homepage parsing failed for {response.url}: {type(e).__name__}: {e}")
            return

        # Extract Logo URL (Heuristic: Standard HTML meta tags)
        logo_url = (
            response.css('link[rel="icon"]::attr(href)').get()
            or response.css('link[rel="shortcut icon"]::attr(href)').get()
        ...
        if result:
            ...
            for link in outlinks:
                ...
                if (
                    ...
                    and link_domain not in IGNORED_DOMAINS
                    and link_domain not in self.seeds
                ):
